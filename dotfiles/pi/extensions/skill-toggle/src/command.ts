...
import type { ApplyResult, SkillRecord, SkillToggleUiResult } from "./types.ts";

export type SkillToggleUiRunner = (ctx: ExtensionCommandContext, skills: SkillRecord[]) => Promise<SkillToggleUiResult>;

// The overlay is injected rather than imported here so this file — which owns
// the apply-and-reload decision — can be exercised without loading the TUI.
export interface ToggleSkillsCommandDeps {
  inventory: SkillInventory;
  planner: SkillTogglePlanner;
  writer: SkillChangeWriter;
  showUi: SkillToggleUiRunner;
}
...
export async function runToggleSkillsCommand(ctx: ExtensionCommandContext, deps: ToggleSkillsCommandDeps): Promise<void> {
  ...
  const result = await deps.showUi(ctx, skills);
