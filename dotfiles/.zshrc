...
# How this shell was reached, for the tmux status bar.
#
# lazyincus sets DEV_VIA on the way in and each hop appends itself, so by the
# time a prompt appears it reads something like `tui>ssh` or `cli>exec`. Unset
# on the workstation and on a plain `ssh box`, which is the point: an empty
# breadcrumb means nothing set this session up, so nothing can be blamed for it.
#
# tmux is the last hop, added here because this is the first thing that knows
# it is inside one. The %>tmux strip keeps re-sourcing .zshrc idempotent.
if [ -n "$DEV_VIA" ] && [ -n "$TMUX" ]; then
  export DEV_VIA="${DEV_VIA%>tmux}>tmux"
  # -p is pane-scoped, so a pane opened over ssh and one opened through
  # `incus exec` each keep their own trail rather than the last one winning.
